...
function RoomEmptyException(roomId) {
    this.name = "RoomEmptyException";
    this.message = "Room '" + roomId + "' is empty";
}

function RoomNoEmptyException(roomId) {
    this.name = "RoomNoEmptyException";
...
RoomService.prototype.removeRoom = function( roomId) {
    ...
    if(!this.isRoomEmpty(roomId))
        throw new RoomNoEmptyException(roomId);
